...
//  GP
//
//  Created by Nakyung Lim on 2017. 6. 14..
...
// User Agent 타입
enum UserAgentType {
    case standard // 기본 user agent + 네이버 표준. ex) Mozilla/5.0 (iPhone; CPU iPhone OS 10_3 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) Mobile/14E269 NAVER(inapp; gp; 100; 0.3.2)
    case splog    // SPLog용. ex) GP/iOS/0.3.0 iOS 10.2; iPhone
}

let GPDefaultUserAgentKey = "GPdefaultUserAgentKey"

extension String {
    
    /// UserAgent string을 생성한다.
    ///
    /// - Parameter type: type1은 GP API, 인앱 페이지용 네이버 표준 user agent. ex: GP/iOS/0.3.0 iOS 10.2; iPhone
    ///                   type2은 SPLog용. ex: nApps(iOS 10.2; iPhone; GP; 0.1.2)
    static func createUserAgent(type: UserAgentType = .standard) -> String {
        switch type {
        case .standard:
            return String.standardUserAgent
            
        case .splog:
            return String.spLogUserAgnet
        }
    }
    
    /// Random string을 생성한다.
    ///
    /// - Parameter length: length길이만큼의 String을 생성
    ...
    /// API, 인앱에 사용하는 네이버 표준 UserAgent 생성
    ///
    /// - Returns: Mozilla/5.0 (iPhone; CPU iPhone OS 10_3 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) Mobile/14E269 NAVER(inapp; gp; 100; 0.3.2)
    private static var standardUserAgent: String {
        // 재현될 일은 없겠지만 앱 버전 스트링이 nil이면 임의로 0.0.0 버전 사용
        let version = SwifterSwift.appVersion ?? "0.0.0"
        let naverUserAgent = "NAVER(inapp; gp; 100; \(version))"
        
        guard let defaultUserAgent = String.getDefaultUserAgent else { return naverUserAgent }
        return defaultUserAgent + " " + naverUserAgent
    }
    
    /// SPLog 에서 사용하는 UserAgent 생성
    ///
    /// - Returns: nApps(iOS 10.2; iPhone; GP; 0.1.2) 형태의 UserAgent
    private static var spLogUserAgnet: String {
        let osInfo = UIDevice.current.systemName + " " + SwifterSwift.systemVersion
        let deviceName = SwifterSwift.deviceModel
        let appName = "70DAYS"
        let appVersion = SwifterSwift.appVersion ?? "0.0"
        
        return "nApps(\(osInfo); \(deviceName); \(appName); \(appVersion))"
    }
    
    /// SPLog, LCSRequest에서 client url 인코딩
    ///
    /// - Returns: dot(.) 과 알파벳, 숫자를 제외한 나머지 캐릭터를 전부 인코딩한 결과를 리턴한다.
    func clientUrlEncoded() -> String {
        return addingPercentEncoding(withAllowedCharacters: CharacterSet(charactersIn: ".").union(.alphanumerics)) ?? self
    }
    
    /// 브라우저 기본 userAgent 세팅
    static func setDefaultUserAgent(_ userAgent: String) {
        UserDefaults.standard.set(userAgent, forKey: GPDefaultUserAgentKey)
        UserDefaults.standard.synchronize()
    }
    
    static var getDefaultUserAgent: String? {
        guard let userAgent = UserDefaults.standard.string(forKey: GPDefaultUserAgentKey), userAgent.count > 0 else { return nil }
...
//        NotificationCenter.default.addObserver(forName: NSNotification.Name.UIApplication.userDidTakeScreenshotNotification, object: nil, queue: mainQueue) { notification in
//            action()
//        }
    }
    #endif
...
//        let type = type(of: )
        return String.init(describing: object.self)
    }
